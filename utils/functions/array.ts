// 数组中找到最大值，和最小值，并返回序号
export function findValueAndIndex(arr:[], option:string) {
...
//判断数组是否为空
export const isNotEmpty = (array: any[]): boolean => Array.isArray(array) && array.length > 0;

// 合并数组
export const merge = (a:[], b:[]) => [...a, ...b]

// 上传图片为，转化fileList用的格式[{name:'',url:''}]
export function arrayTransformFileList(arr:[any]) {
    return arr.map((item) => ({
        name: item?.name,
...
// 数组拼接前缀
export function convertUrlsToString(urls:[any], prefix:string) {
...
// 不修改保存的时候去前辍
// 修改了，则又要用初次上传的方法改结构
export function arrayTransformFileListToString(arr:[any],prefix:string) {
...
export function arrayTransformFileListToArray(arr:[any],prefix:string) {
    if (Array.isArray(arr)) {
        return arr
            .map((item) => {
                if (item?.response?.data) {
